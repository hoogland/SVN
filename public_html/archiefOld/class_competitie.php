<?php
    class competitie{
        ...
        function stand($spelers, $partijen)                  //GENEREREN STAND
        {
            ...
            $stand = array_reverse($this->multisort($stand,array("TPR","PUNTEN","SB", "ONDERLING")));
        ...
        function partijen_speler_print($spelers, $partijen, $speler, $competitie)
        {
            ...
            $competitie_gegevens = $this->competitie_gegevens($competitie);
            ...
            if(count($deelnemers) > 0)
            {
                ...
                foreach($deelnemers as $key => $tegenstander)
                {
                    ...
                    */
                    ...
                    $kleuren = $this->kleur(array(array($speler,$plaats), array($key,$deelnemers[$key]["plaats"])),$competitie_gegevens["aantal_deelnemers"]);
        ...
        function speler($speler)
        {
            ...
            if(count($partijen) == 0)
            {echo "Geen interne partijen gevonden";  }
            ...
            //Externe competitie
            $sql = "SELECT count(*) as aantal, SUM(score) as score, AVG(spelerElo) as speler, AVG(tegenstanderElo) as tegenstander FROM svn_extern_partijen WHERE speler = ".$speler;
            $result = mysql_query($sql);
            if(mysql_num_rows($result) > 0)
            {
                $totaal = mysql_fetch_assoc($result);
                $sql = "SELECT count(*) as aantal, SUM(score) as score, AVG(spelerElo) as speler, AVG(tegenstanderElo) as tegenstander FROM svn_extern_partijen WHERE speler = ".$speler." AND kleur = 1";
                $result = mysql_query($sql);
                $wit = mysql_fetch_assoc($result);
                $sql = "SELECT count(*) as aantal, SUM(score) as score, AVG(spelerElo) as speler, AVG(tegenstanderElo) as tegenstander FROM svn_extern_partijen WHERE speler = ".$speler." AND kleur = 2";
                $result = mysql_query($sql);
                $zwart = mysql_fetch_assoc($result);
                echo "<h2>Externe score</h2>";
                echo "<table><tr><td></td><td>Aantal Partijen</td><td>Score</td><td>Percentage</td><td>Gemiddelde Rating</td><td>Gemiddelde rating tegenstander</td><td>TPR</td>";
                echo "<tr><td>Totaal</td><td>".$totaal["aantal"]."</td><td>".$totaal["score"]."</td><td>".round($totaal["score"]/$totaal["aantal"]*100,2)."%</td><td>".round($totaal["speler"])."</td><td>".round($totaal["tegenstander"])."</td><td>".round($totaal["tegenstander"] - 400 * log10(1 / ($totaal["score"]/$totaal["aantal"]) - 1))."</td>";
                echo "<tr><td>Wit</td><td>".$wit["aantal"]."</td><td>".$wit["score"]."</td><td>".round($wit["score"]/$wit["aantal"]*100,2)."%</td><td>".round($wit["speler"])."</td><td>".round($wit["tegenstander"])."</td><td>".round($wit["tegenstander"] - 400 * log10(1 / ($wit["score"]/$wit["aantal"]) - 1))."</td>";
                echo "<tr><td>Zwart</td><td>".$zwart["aantal"]."</td><td>".$zwart["score"]."</td><td>".round($zwart["score"]/$zwart["aantal"]*100,2)."%</td><td>".round($zwart["speler"])."</td><td>".round($zwart["tegenstander"])."</td><td>".round($zwart["tegenstander"] - 400 * log10(1 / ($zwart["score"]/$zwart["aantal"]) - 1))."</td>";
                echo "</table>";
        ...
        function competitie_gegevens($comp_id)
        {
            ...
            $datum = date("d-m-Y", $datum);
            $competitie['laatste_ronde_datum'] = $datum;

            //Aantal spelers
            $deelnemers = $this->spelers_deelname($comp_id);
            $competitie['aantal_deelnemers'] = count($deelnemers);
        ...
        function rating_rapportage($competitie_id, $van, $tot)
        {
            ...
            echo "052 ".$competitie["laatste_ronde_datum"]."\r\n";   // DATUM LAATSTE RONDE
        ...
        function eindscore($competitie)
        {
            ...
            //Played matches
            $played;
            foreach($result["partijen"] as $match)
                $played[] = array("wit" => $match["speler_wit"],"zwart" =>  $match["speler_zwart"]);

            //Create participants table
            $positions;
            foreach($deelnemers as $key => $deelnemer)
                $positions[] = array($key, $deelnemer["plaats"]);

            //ADD Still to play
            $to_play;
            for($wit = 0; $wit < count($deelnemers) - 1; $wit++)
            {
                for($zwart = $wit + 1; $zwart < count($deelnemers); $zwart++)
                {
                    $colours = $this->kleur(array($positions[$wit], $positions[$zwart]),count($deelnemers));

                    if(!in_array($colours,$played) && $colours["wit"] != 76 &&  $colours["zwart"] != 76)
                        $to_play[] = $colours; 
            ...
            {
                $scenarios[] = $scenario;
                $scenario[0] = $scenario[0] + 0.5;
                for($a = 0; $a < count($scenario); $a++)
                {
                    if($scenario[$a] > 1 && $a < count($scenario) - 1)
                    {
                        $scenario[$a + 1] = $scenario[$a + 1] + 0.5;
                        $scenario[$a] = 0;
                    }
                    elseif($scenario[$a] > 1 && $a = count($scenario) - 1)
                        $complete = true;
                } 
            }
            /*    echo "<table>";
            for($a = 0; $a < count($scenarios); $a++)
            {
            echo "<TR>";
            for($b = 0; $b < count($scenarios[$a]); $b++)
            echo "<TD>".$scenarios[$a][$b];
            }
            ...
            $eindresultaten;
            $d = 0;        
            $lines = ceil(count($scenarios) / (50 * 30));           

            foreach($scenarios as $scenario)
            {
                $id = 2664;
                $matches = $result["partijen"];
                for($a = 0; $a < count($to_play); $a++)
                {
                    $id++;
                    $score = 1;
                    ...
                    $matches[] = array(
                    0 => $to_play[$a]["wit"],
                    "speler_wit" => $to_play[$a]["wit"],
                    1 => $to_play[$a]["zwart"], 
                    "speler_zwart" => $to_play[$a]["zwart"], 
                    3 => $deelnemers[$to_play[$a]["wit"]]["rating"], 
                    "rating_wit" => $deelnemers[$to_play[$a]["wit"]]["rating"], 
                    4 => $deelnemers[$to_play[$a]["zwart"]]["rating"], 
                    "rating_zwart" => $deelnemers[$to_play[$a]["zwart"]]["rating"], 
                    "uitslag" => $score, 
                    2 => $scenario[$a],
                    "ronde" => $id,
                    5 => $id);     
                }  
                $stand = $this->stand($result["spelers"], $matches);
                for($a = 0; $a < count($stand); $a++)
                    $eindresultaten[$stand[$a]["speler"]][$a]++;
                $d++;
                if($d % 50 == 0)
                {
                    echo "|";flush();
            ...
            echo "<table>";
            foreach($eindresultaten as $key => $resultaat)
            {
                echo "<TR><TD>".$deelnemers[$key]["naam"];
                for($a = 0; $a < count($deelnemers); $a++)
                    echo "<TD>".$resultaat[$a]; 
            }
            echo "</table>";
